...
const char *const SERVER2_METHOD_MULTI_NAME = "multi";
...
int main(int argc, char **argv) {
    ...
    // Multi method from server2
    printf("Enter two numbers to be multiplied: ");
    scanf("%d %d", &a, &b);

    msg = dbus_message_new_method_call(SERVER2_BUS_NAME, SERVER2_OBJECT_PATH_NAME, SERVER2_INTERFACE_NAME, SERVER2_METHOD_MULTI_NAME);
    dbus_message_iter_init_append(msg, &args);
    dbus_message_iter_append_basic(&args, DBUS_TYPE_INT32, &a);
    dbus_message_iter_append_basic(&args, DBUS_TYPE_INT32, &b);
