...
const char *const BUS_NAME = "in.softprayog.server2";
const char *const OBJECT_PATH_NAME = "/in/softprayog/server2";
const char *const INTERFACE_NAME = "in.softprayog.server2.interface";
const char *const METHOD_MULTI_NAME = "multi";
const char *const METHOD_DIV_NAME = "div";
...
DBusHandlerResult handle_message(DBusConnection *conn, DBusMessage *msg, void *data) {
    ...
    if (dbus_message_is_method_call(msg, INTERFACE_NAME, METHOD_MULTI_NAME)) {
    ...
    } else if (dbus_message_is_method_call(msg, INTERFACE_NAME, METHOD_DIV_NAME)) {
        dbus_message_iter_init(msg, &args);
        dbus_message_iter_get_basic(&args, &a);
        dbus_message_iter_next(&args);
        dbus_message_iter_get_basic(&args, &b);
        ...
        if (b == 0) {
            snprintf(str, sizeof(str), "Division by zero");
        } else {
            snprintf(str, sizeof(str), "%.2f", (float)a / (float)b);
        }
